...
pub(super) fn notify_open(lsp_state: &LspState, content: &TabContent) {
    ...
    let client = lsp_state.client.clone();
    let lang_id = language_id;
    lsp_state.tokio_runtime.spawn(async move {
...
pub(super) fn notify_change(lsp_state: &LspState, content: &TabContent) {
    // 1. 映射语言到 LSP language_id（无配置则跳过）
    let language_id = match language_to_lsp_id_opt(content.language) {
        Some(id) => id.to_string(),
        None => return,
...
pub(crate) fn request_completion(lsp_state: &mut LspState, content: &TabContent) {
    let language_id = match language_to_lsp_id_opt(content.language) {
        Some(id) => id.to_string(),
        None => return,
    };
    ...
    // 记录触发位置，用于弹窗定位
    lsp_state.completion_trigger_line = content.cursor_line;
    lsp_state.completion_trigger_col = content.cursor_col;
    ...
    match event {
        LspEvent::Diagnostics { uri, diagnostics } => {
            ...
            if diagnostics.is_empty() {
                lsp_state.diagnostics.remove(&uri);
            } else {
                lsp_state.diagnostics.insert(uri, diagnostics);
            }
            // 状态栏提示诊断数量（仅当前文件）
            let mut status = None;
            if let Some(path) = current_path {
                if let Ok(current_uri) = Url::from_file_path(path) {
                    let current_count = lsp_state
                        .diagnostics
                        .get(&current_uri)
                        .map(|v| v.len())
                        .unwrap_or(0);
        ...
        LspEvent::Completion { uri, items } => {
            if items.is_empty() {
                lsp_state.completion_visible = false;
            } else {
                // 验证是当前文件的补全结果
                let is_current = current_path
                    ...
                    .unwrap_or(false);
                if is_current {
                    lsp_state.completion_items = items;
                    lsp_state.completion_selected = 0;
                    lsp_state.completion_visible = true;
                }
            }
            None
        }
        ...
        LspEvent::Hover { uri, hover } => {
            let is_current = current_path
                ...
                .unwrap_or(false);
            if is_current {
                lsp_state.hover_content = extract_hover_text(&hover);
            }
            None
        }
        // 未接线的 LSP 事件静默忽略
...
// ===== Phase H2: 补全弹窗导航 =====
/// 补全列表下一项（↓ 键）
pub(crate) fn completion_next(lsp_state: &mut LspState) {
...
/// 补全列表上一项（↑ 键）
pub(crate) fn completion_prev(lsp_state: &mut LspState) {
    if !lsp_state.completion_visible || lsp_state.completion_items.is_empty() {
        return;
    }
    if lsp_state.completion_selected == 0 {
        lsp_state.completion_selected = lsp_state.completion_items.len() - 1;
    } else {
        lsp_state.completion_selected -= 1;
    }
}

/// 关闭补全弹窗（Esc 键 或 失焦）
pub(crate) fn completion_cancel(lsp_state: &mut LspState) {
    if lsp_state.completion_visible {
        lsp_state.completion_visible = false;
        lsp_state.completion_items.clear();
    }
}

/// 关闭悬停 tooltip
#[allow(dead_code)]
pub(crate) fn hover_cancel(lsp_state: &mut LspState) {
    lsp_state.hover_content = None;
}

/// 初始化 LSP 客户端（在打开工作区文件夹时调用）
pub fn init(lsp_state: &mut LspState, root_dir: &Path) {
    // 如果已有 LSP 客户端，先清理
    lsp_state.legacy_lsp_client = None;
    lsp_state.rx = None;
    lsp_state.legacy_runtime = None;
    ...
    if is_rust_workspace {
        if let Some(config) = aether_lsp::client::default_server_config("rust") {
            ...
            lsp_state.legacy_lsp_client = Some(client_clone);
        } else {
            lsp_state.legacy_lsp_client = Some(std::sync::Arc::new(client));
        }
    } else {
        lsp_state.legacy_lsp_client = Some(std::sync::Arc::new(client));
    }

    lsp_state.rx = Some(rx);
    lsp_state.legacy_runtime = Some(runtime);
}

/// 通知 LSP 服务器文档已打开
pub fn open_document(lsp_state: &LspState, language: Language, path: &Path, text: &str) {
    let Some(client) = lsp_state.legacy_lsp_client.clone() else {
        return;
    };
    let Some(runtime) = lsp_state.legacy_runtime.as_ref() else {
        return;
    };
...
pub fn poll_events(
    ...
    let Some(rx) = lsp_state.rx.as_mut() else {
        return false;
    };
    while let Ok(event) = rx.try_recv() {
        ...
        match event {
            LspEvent::Diagnostics {
                ...
                if let Some(client) = lsp_state.legacy_lsp_client.as_ref() {
...
impl LspState {
    ...
    pub(crate) fn request_hover(&mut self, content: &TabContent, line: usize, col: usize) {
        request_hover(self, content, line, col)
    }
    ...
    pub fn open_document(&self, language: Language, path: &Path, text: &str) {
        open_document(self, language, path, text)
    }
    ...
    pub fn poll_events(
        ...
        poll_events(self, diagnostics, status_message)
    }
}

/// 冰冻态：关停全部 LSP 服务器子进程释放内存（rust-analyzer 可达 GB 级）。
/// 诊断/补全数据一并清空（预期行为，重启后自动恢复）。
pub(crate) fn freeze_lsp(state: &mut EditorState) {
    if state.lsp.lsp.frozen {
        return;
    }
    state.lsp.lsp.frozen = true;
    // legacy 工作区级客户端（rust-analyzer）优雅关停
    if let (Some(client), Some(runtime)) = (
        state.lsp.lsp.legacy_lsp_client.clone(),
        state.lsp.lsp.legacy_runtime.as_ref(),
    ) {
        runtime.spawn(async move {
    ...
    // 按文档启动的服务器优雅关停
    let client = state.lsp.lsp.client.clone();
    state.lsp.lsp.tokio_runtime.spawn(async move {
        let _ = client.shutdown_all().await;
    });
...
pub(crate) fn thaw_lsp_on_demand(state: &mut EditorState) {
    if !state.lsp.lsp.frozen {
        return;
    }
    state.lsp.lsp.frozen = false;
    state.ui.status_message = "语言服务恢复中…".to_string();
    // 工作区级 rust-analyzer 重新初始化（内部按需拉起）
    if let Some(folder) = state.fs.current_folder.clone() {
        state.lsp.lsp.init(&folder);
    }
...
pub(crate) fn lsp_notify_change_thawed(state: &mut EditorState) {
    if state.lsp.lsp.frozen {
        thaw_lsp_on_demand(state);
        return;
    }
    state.lsp.lsp.notify_change(&state.editor.content);
}

/// 接受当前选中的补全项（Enter 键）。
/// 将光标移回触发位置，删除已输入的过滤文本，插入补全项的 insert_text 或 label。
pub(crate) fn completion_accept(state: &mut EditorState) {
    if !state.lsp.lsp.completion_visible || state.lsp.lsp.completion_items.is_empty() {
        return;
    }
    let item = state.lsp.lsp.completion_items[state.lsp.lsp.completion_selected].clone();
    ...
    state.lsp.lsp.completion_items.clear();
    // 将光标移回触发位置
    if state.editor.content.cursor_line != state.lsp.lsp.completion_trigger_line {
        return; // 跨行编辑，放弃插入
    }
...
impl EditorState {
    ...
    pub(crate) fn thaw_lsp_on_demand(&mut self) {
        thaw_lsp_on_demand(self)
    }

    /// 编辑后通知 LSP；冰冻态先解冻重启服务器。
    /// 解冻路径内 notify_open 已全量宣告最新内容，无需再发 did_change。
    pub(crate) fn lsp_notify_change_thawed(&mut self) {
        lsp_notify_change_thawed(self)
    }
