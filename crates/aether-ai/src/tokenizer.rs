...
impl TokenUsage {
    ...
    pub fn calculate_cost(&self, pricing: &ModelPricing) -> f64 {
        ...
        let cached_input_cost = (self.cached_input_tokens as f64 / 1000.0) * pricing.cached_input_price_per_1k;
...
impl DeepSeekTokenizer {
    ...
    fn get_tokenizer_path() -> Result<PathBuf, Box<dyn std::error::Error>> {
        ...
        let tokenizer_path = current_dir.join("deepseek_v3_tokenizer").join("tokenizer.json");
    ...
    /// 计算消息列表的 token 数量
    pub fn count_messages_tokens(&self, messages: &[crate::ChatMessage]) -> Result<usize, Box<dyn std::error::Error>> {
...
/// 全局 tokenizer 实例（懒加载）
static TOKENIZER: std::sync::OnceLock<Result<DeepSeekTokenizer, String>> = std::sync::OnceLock::new();
...
pub fn count_messages_tokens(messages: &[crate::ChatMessage]) -> Result<usize, Box<dyn std::error::Error>> {
