...
## 更新摘要
**变更内容**   
- 扩展了Ctrl+快捷键系统，新增终端清屏、AI面板切换、设置面板等功能
- 增强了上下文感知的按键绑定，改进了不同UI状态下的按键处理优先级
- 优化了字符输入处理，增强了UTF-16代理对支持和IME集成
- 完善了终端面板的键盘路由机制，支持更复杂的编辑操作
...
**更新** 本次更新重点增强了快捷键支持系统，增加了上下文感知的按键绑定和增强的字符输入处理能力。
...
**图表来源**
- [crates/aether-win32/src/window/window_messages.rs:1-200](file://crates/aether-win32/src/window/window_messages.rs#L1-L200)
...
- [crates/aether-win32/src/window/keyboard_handler/key_down_ctrl.rs:1-845](file://crates/aether-win32/src/window/keyboard_handler/key_down_ctrl.rs#L1-L845)
- [crates/aether-win32/src/window/keyboard_handler/key_down_edit.rs:1-791](file://crates/aether-win32/src/window/keyboard_handler/key_down_edit.rs#L1-L791)
- [crates/aether-win32/src/window/keyboard_handler/char_input.rs:1-528](file://crates/aether-win32/src/window/keyboard_handler/char_input.rs#L1-L528)
- [crates/aether-win32/src/window/ime_handler.rs:1-132](file://crates/aether-win32/src/window/ime_handler.rs#L1-L132)
- [crates/aether-win32/src/ime.rs:1-315](file://crates/aether-win32/src/ime.rs#L1-L315)
...
- **增强版** Ctrl+ 快捷键总线：集中处理 Ctrl 组合键，包括文件操作、视图切换、缩放、剪贴板、查找/撤销重做、标签页导航、词级移动、列光标、终端清屏、AI面板切换等新功能。
- 非 Ctrl 编辑键：处理回车、退格、删除、方向键、Home/End/PageUp/Down、Tab 等，并在终端聚焦时优先转发至 ConPTY。
- **增强版** WM_CHAR 字符分发：处理 UTF-16 代理对，过滤可打印字符，按优先级路由到各输入目标（文件树、设置、搜索、SSH/克隆、命令面板、查找替换、终端、AI 面板），最终广播到编辑器。
...
**更新** 本次更新显著增强了快捷键系统和字符处理能力，特别是在上下文感知和用户体验方面。
...
**图表来源**
- [crates/aether-win32/src/keyboard_hook.rs:150-315](file://crates/aether-win32/src/keyboard_hook.rs#L150-L315)
- [crates/aether-win32/src/window/window_messages.rs:1-200](file://crates/aether-win32/src/window/window_messages.rs#L1-L200)
- [crates/aether-win32/src/window/keyboard_handler/key_down.rs:18-182](file://crates/aether-win32/src/window/keyboard_handler/key_down.rs#L18-L182)
- [crates/aether-win32/src/window/keyboard_handler/key_down_ctrl.rs:14-845](file://crates/aether-win32/src/window/keyboard_handler/key_down_ctrl.rs#L14-L845)
- [crates/aether-win32/src/window/keyboard_handler/key_down_edit.rs:14-791](file://crates/aether-win32/src/window/keyboard_handler/key_down_edit.rs#L14-L791)
- [crates/aether-win32/src/window/keyboard_handler/char_input.rs:11-528](file://crates/aether-win32/src/window/keyboard_handler/char_input.rs#L11-L528)
- [crates/aether-win32/src/window/ime_handler.rs:10-132](file://crates/aether-win32/src/window/ime_handler.rs#L10-L132)
- [crates/aether-win32/src/ime.rs:29-315](file://crates/aether-win32/src/ime.rs#L29-L315)
...
- 若 Ctrl 按下：
  ...
  - 否则进入 **增强版** Ctrl+ 快捷键总线 okd_ctrl_dispatch，按功能分组处理。
...
FileTreeActive --> |否| CtrlDispatch["okd_ctrl_dispatch 分发增强版"]
...
- 快捷键总线 okd_ctrl_dispatch 将 Ctrl+ 组合键按功能分组：
  - **新增功能**：终端清屏（Ctrl+L）、AI面板切换（Ctrl+Shift+A）、设置面板（Ctrl+,）
  - 文件操作：打开文件/文件夹、保存/另存为、新建项目
  - 视图切换：侧边栏、命令面板、底部终端面板、资源管理器视图、源代码管理视图
  - 字体缩放：放大/缩小/重置（支持图片预览模式）
  - 剪贴板：复制/剪切/粘贴/全选（终端聚焦时 Ctrl+C 中断进程）
  - 查找/替换/撤销/重做：Ctrl+F/H/Z/Y（增强撤销逻辑，支持文件删除撤销）
  - 标签页：切换/关闭/恢复最后关闭的标签
...
**更新** 新增了多个实用快捷键功能，特别是终端清屏、AI面板切换和增强的撤销逻辑。
...
### **增强版** WM_CHAR 字符输入与 UTF-16 代理对处理
...
- **增强版** 按优先级分发到各输入目标：文件树输入、设置字段、沙盒评测、搜索面板、SSH/克隆对话框、新建项目、SSH 管理、命令面板、查找替换、终端、历史浮窗、AI 面板，最后广播到编辑器。
...
IMECompose --> |是| CEnd
IMECompose --> |否| Route["按优先级路由到各输入目标增强版"]
...
**更新** 增强了字符输入处理，新增了多个输入目标的优先级处理，改进了UTF-16代理对的支持。
...
章节来源
- [crates/aether-win32/src/focus_manager.rs:40-193](file://crates/aether-win32/src/focus_manager.rs#L40-L193)
- [crates/aether-win32/src/window/window_messages.rs:1-200](file://crates/aether-win32/src/window/window_messages.rs#L1-L200)
...
```mermaid
graph LR
KD["key_down.rs"] --> KC["key_down_ctrl.rs增强版"]
KD --> KE["key_down_edit.rs"]
KD --> IMH["ime_handler.rs"]
CH["char_input.rs增强版"] --> ES["editor.rs"]
KC --> ES
KE --> ES
IMH --> IME["ime.rs"]
...
**图表来源**
- [crates/aether-win32/src/window/keyboard_handler/key_down.rs:1-182](file://crates/aether-win32/src/window/keyboard_handler/key_down.rs#L1-L182)
- [crates/aether-win32/src/window/keyboard_handler/key_down_ctrl.rs:1-845](file://crates/aether-win32/src/window/keyboard_handler/key_down_ctrl.rs#L1-L845)
- [crates/aether-win32/src/window/keyboard_handler/key_down_edit.rs:1-791](file://crates/aether-win32/src/window/keyboard_handler/key_down_edit.rs#L1-L791)
- [crates/aether-win32/src/window/keyboard_handler/char_input.rs:1-528](file://crates/aether-win32/src/window/keyboard_handler/char_input.rs#L1-L528)
- [crates/aether-win32/src/window/ime_handler.rs:1-132](file://crates/aether-win32/src/window/ime_handler.rs#L1-L132)
...
- **更新** 增强了撤销操作的智能判断，减少了不必要的状态检查和重绘。
...
- **更新** 快捷键冲突：审查 Ctrl+ 快捷键分组函数，确保特定场景（如文件树输入框激活）吞掉快捷键；检查新增功能的快捷键是否与现有功能冲突。
- 焦点丢失导致输入错误：确认 WM_SETFOCUS/WM_KILLFOCUS 同步了 FocusManager 状态。
- **更新** 字符输入异常：检查UTF-16代理对处理逻辑，确认高代理和低代理的正确配对。
...
- **增强版** 完善的组合键与快捷键分发机制，包含新增的实用功能
- 全面的 IME 集成与多语言输入支持
- 终端面板在 IME 环境下的正确编辑键路由
- 统一的焦点管理与事件冒泡控制
- 面向性能的轻量处理路径与异步任务协调

**更新总结** 本次更新显著增强了快捷键支持系统，增加了上下文感知的按键绑定和增强的字符输入处理能力，提供了更好的用户体验和功能完整性。该架构具备良好的扩展性与可维护性，便于后续增加新的快捷键、UI 面板与输入法特性。
...
- **更新** 常用快捷键参考（部分）：
  ...
  - Ctrl+=/-/0：字体缩放/重置（支持图片预览模式）
  - Ctrl+C/X/V/A：复制/剪切/粘贴/全选（终端聚焦时 Ctrl+C 中断）
  - Ctrl+F/H/Z/Y：查找/替换/撤销/重做（增强撤销逻辑）
  ...
  - **新增** Ctrl+L：终端清屏（发送 Form Feed 给 shell）
  - **新增** Ctrl+Shift+A：切换右侧 AI 面板
  - **新增** Ctrl+,：打开设置标签页
  - **新增** Ctrl+J：切换底部面板
  - **新增** Ctrl+Shift+E：切换到资源管理器视图
  - **新增** Ctrl+Shift+V：切换 Markdown 预览模式
  - **新增** Ctrl+G：打开命令面板（带 : 前缀）或切换到源代码管理视图

**更新** 新增了多个实用的快捷键功能，提升了工作效率和用户体验。

章节来源
- [crates/aether-win32/src/window/keyboard_handler/key_down_ctrl.rs:124-845](file://crates/aether-win32/src/window/keyboard_handler/key_down_ctrl.rs#L124-L845)