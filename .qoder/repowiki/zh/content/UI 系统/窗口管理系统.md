...
## 更新摘要
**变更内容**   
- 新增欢迎页面Activity Bar集成章节，说明固定活动栏和侧边栏切换功能
- 更新布局计算章节，反映欢迎页场景下的特殊处理逻辑
- 增强性能优化章节，包含Activity Bar相关的渲染优化
- 完善窗口状态管理，支持欢迎页期间的Activity Bar持久化
...
**更新** 本次更新重点增强了欢迎页面的Activity Bar集成，实现了固定的活动栏和侧边栏切换功能，改进了布局计算和性能优化。
...
- 设置与持久化由共享配置模块提供
- Activity Bar和布局管理提供统一的界面区域计算和交互逻辑
...
I --> J["Activity Bar管理<br/>activity_bar.rs"]
I --> K["布局计算<br/>layout.rs"]
I --> L["欢迎页面渲染<br/>welcome.rs"]
```
...
- [activity_bar.rs:1-256](file://crates/aether-win32/src/activity_bar.rs#L1-L256)
- [layout.rs:266-729](file://crates/aether-win32/src/layout.rs#L266-L729)
...
- **新增** Activity Bar管理：提供活动栏项的增删改查、拖拽重排、状态持久化等功能。
- **新增** 布局管理器：统一管理窗口各区域的几何计算，支持欢迎页特殊场景处理。
...
- [activity_bar.rs:21-186](file://crates/aether-win32/src/activity_bar.rs#L21-L186)
- [layout.rs:266-729](file://crates/aether-win32/src/layout.rs#L266-L729)
...
participant Layout as "布局管理<br/>layout.rs"
participant ActBar as "活动栏<br/>activity_bar.rs"
...
### 欢迎页面Activity Bar集成（新增）
**更新** 欢迎页面集成了Activity Bar功能，实现了固定的活动栏和侧边栏切换功能。

#### Activity Bar核心功能
- **固定显示**：在欢迎页期间，活动栏固定常驻，作为侧边栏功能按钮的入口，不可关闭。
- **视图切换**：支持在工作区、源代码管理、远程资源管理等视图间切换。
- **拖拽重排**：支持长按进入自定义模式，拖拽调整活动栏项的顺序。
- **状态持久化**：活动栏的显示状态、顺序和活跃项都会自动保存和恢复。

#### 布局计算优化
- **欢迎页抑制**：在欢迎页期间，侧边栏面板默认隐藏，用户可通过活动栏图标或快捷键调起。
- **区域计算**：活动栏始终占位，但侧边栏仅在用户主动调起时才占用空间。
- **动画支持**：侧边栏展开/收起支持200ms平滑动画，提升用户体验。

```mermaid
sequenceDiagram
participant User as "用户"
participant ActBar as "活动栏"
participant Layout as "布局管理器"
participant Sidebar as "侧边栏"
User->>ActBar : 点击活动栏图标
ActBar->>Layout : toggle_sidebar()
Layout->>Sidebar : 切换可见性
Sidebar->>Sidebar : 启动展开/收起动画
Sidebar-->>User : 显示/隐藏侧边栏面板
```

图示来源
- [activity_bar.rs:21-186](file://crates/aether-win32/src/activity_bar.rs#L21-L186)
- [layout.rs:647-686](file://crates/aether-win32/src/layout.rs#L647-L686)
- [welcome.rs:139-150](file://crates/aether-win32/src/welcome.rs#L139-L150)
...
### 布局管理器增强（新增）
**更新** 布局管理器现在支持欢迎页的特殊场景处理，包括活动栏固定显示和侧边栏智能抑制。

#### 核心特性
- **欢迎页模式**：`welcome_active` 标志控制欢迎页状态，影响活动栏显示和侧边栏行为。
- **智能抑制**：`welcome_sidebar_suppressed()` 方法判断是否抑制侧边栏渲染。
- **区域计算**：所有区域计算方法都考虑了欢迎页状态，确保正确的布局计算。
- **动画支持**：侧边栏展开/收起支持平滑动画，提升视觉体验。

```mermaid
flowchart TD
Welcome["欢迎页激活"] --> CheckActive{"活动栏是否显示?"}
CheckActive --> |是| ShowActBar["显示活动栏"]
CheckActive --> |否| HideActBar["隐藏活动栏"]
ShowActBar --> CheckSidebar{"侧边栏是否被抑制?"}
HideActBar --> CheckSidebar
CheckSidebar --> |是| HideSidebar["隐藏侧边栏"]
CheckSidebar --> |否| ShowSidebar["显示侧边栏"]
HideSidebar --> CalcRegions["计算各区域"]
ShowSidebar --> CalcRegions
CalcRegions --> UpdateLayout["更新布局状态"]
```

图示来源
- [layout.rs:389-401](file://crates/aether-win32/src/layout.rs#L389-L401)
- [layout.rs:424-476](file://crates/aether-win32/src/layout.rs#L424-L476)
- [layout.rs:647-686](file://crates/aether-win32/src/layout.rs#L647-L686)
...
- 模块耦合：
  ...
  - **新增** activity_bar.rs 依赖 layout.rs（布局常量）和 icons.rs（图标资源）。
  - **新增** layout.rs 提供统一的布局计算接口，被多个模块依赖。
...
Msg --> Layout["layout.rs"]
Msg --> ActBar["activity_bar.rs"]
ActBar --> Layout
ActBar --> Icons["icons.rs"]
```
...
- **新增** Activity Bar优化：欢迎页期间活动栏固定显示，避免频繁的显示/隐藏操作；侧边栏智能抑制减少不必要的渲染。
- **新增** 布局计算优化：使用懒加载和缓存机制，避免每帧重复计算布局常量。
...
- [activity_bar.rs:21-186](file://crates/aether-win32/src/activity_bar.rs#L21-L186)
- [layout.rs:368-387](file://crates/aether-win32/src/layout.rs#L368-L387)
...
- **新增** Activity Bar问题：检查活动栏状态是否正确持久化；确认欢迎页模式下活动栏固定显示逻辑；验证侧边栏抑制逻辑是否正常工作。
...
## 结论
本窗口管理系统在 Win32 基础上实现了完善的 DPI 感知、多显示器适配、窗口状态持久化与 DWM 视觉效果集成。通过统一的渲染驱动与健壮的消息处理机制，系统在复杂交互场景下保持了稳定性与性能。

**更新** 本次更新重点增强了欢迎页面的Activity Bar集成，实现了固定的活动栏和侧边栏切换功能，改进了布局计算和性能优化。新的Activity Bar系统提供了更好的用户体验，支持拖拽重排、状态持久化和智能的侧边栏抑制逻辑。建议在后续迭代中持续优化高 DPI 下的命中测试精度、进一步细化脏区域追踪粒度，并完善跨实例通信的错误恢复与日志诊断能力。