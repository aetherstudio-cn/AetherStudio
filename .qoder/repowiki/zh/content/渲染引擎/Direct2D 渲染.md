...
## 简介
本技术文档聚焦于 Direct2D 渲染模块，系统阐述 Direct2D 与 DirectWrite 的集成实现、渲染上下文管理、绘制原语封装、图形对象生命周期、画刷缓存系统、文本渲染管线（字体处理、布局、字符宽度测量与 Unicode 支持）、玻璃效果（透明度与混合）的实现原理，并提供创建渲染上下文、绘制图形与处理文本的具体代码路径指引。同时给出性能调优建议与常见问题解决方案。
...
- d2d/brush_cache：画刷与文本格式/布局缓存，避免每帧 COM 对象分配
...
- 画刷缓存：预存常用颜色画刷 + HashMap 回退，控制最大条目数并支持设备丢失清理。
- 文本格式与布局缓存：预置常用 TextFormat；对 IDWriteTextLayout 采用两代淘汰策略（young/old），避免周期性全清导致的掉帧。
...
- 设计要点：
  ...
  - 颜色键使用四通道打包（RGBA 归一化到 0-255 后移位拼接），避免浮点精度问题。
- 生命周期：
...
```mermaid
flowchart TD
Start(["get_brush(color)"]) --> Key["生成颜色键"]
Key --> Pre{"预存命中?"}
Pre -- 是 --> ReturnPre["返回预存画刷"]
Pre -- 否 --> Map{"HashMap 命中?"}
Map -- 是 --> ReturnMap["返回缓存画刷"]
Map -- 否 --> Create["创建新画刷"]
Create --> CheckCap{"容量超限?"}
CheckCap -- 是 --> ClearMap["清空 HashMap"]
...
- 文本布局缓存（TextLayoutCache）：
  - 两代淘汰策略（young/old），热点条目晋升回 young，避免周期性全清导致掉帧。
  - 字体大小变化时自动清空缓存，确保布局一致性。
  - 支持带省略号的布局（侧边栏长文本截断）。
- 文本渲染器（TextRenderer）：
  - 基于 DirectWrite 创建字体格式，实测等宽字符宽度，避免硬编码比例。
  - 支持 DPI 缩放与基础字号调整，动态重建格式并重新测量。
...
## 性能考量
- 画刷缓存：
  - 预存常用颜色画刷，线性扫描命中快；HashMap 回退控制最大条目数，避免内存膨胀。
  - 颜色键使用整型打包，避免浮点精度导致的缓存失效。
- 文本格式与布局缓存：
  - 预置常用 TextFormat，减少频繁创建开销。
  - TextLayoutCache 采用两代淘汰，热点条目自然存活，消除周期性全清导致的掉帧。
...
## 结论
该 Direct2D 渲染模块通过分层设计与缓存策略，有效降低了 COM 对象分配与创建开销，提升了渲染性能与稳定性。结合 DirectWrite 的文本布局与测量能力，实现了高精度的文本渲染与交互体验。玻璃效果通过透明度与混合实现现代 UI 视觉风格。整体架构清晰、职责明确，便于扩展与维护。
