...
## 简介
本系统为编辑器内置的 AI 助手，提供多 AI 提供商统一接入（DeepSeek、Kimi、OpenAI 等）、Agent 模式（对话管理、工具调用、上下文维护）、流式响应与 UI 同步、记忆存储（对话历史、嵌入向量、语义搜索）以及安全与权限控制。通过“热/温/冷”三阶段持久化与 ACE 反思沉淀策略，实现高可用、可演进、可检索的智能体验。
...
- 嵌入模型：ONNX Runtime 加载 bge-small-zh-v1.5，未就绪时回退 n-gram 哈希向量，保证维度一致与链路可用。
...
- AetherDbMemoryStore 实现：基于自研 AetherDB，消息 tag=conv_id，条目 tag=section；HNSW 向量索引；flush/shrink_memory 支持 WAL 与 compaction。
- 嵌入模型：bge-small-zh-v1.5 ONNX 推理，未就绪时回退 n-gram 哈希向量，确保维度一致与检索链路可用。
- 三阶段持久化：
...
- embedding 为 memory_store 与 warm_data 提供向量能力。
...
- 向量检索优化：HNSW 索引 + 小规模退化暴力搜索；混合检索使用子串关键词与向量 RRF 融合，平衡召回与精度。
- 持久化效率：热数据 mmap 增量日志零拷贝追加；温数据批量归档并 flush；shrink_memory 触发 compaction 回收垃圾段。
- 模型加载：ONNX 模型懒加载，未就绪回退 n-gram 哈希，保证检索链路可用且不影响启动速度。
...
- 向量维度不匹配：AetherDbMemoryStore.check_dim 校验 embedding 维度，防止索引异常。
...
## 结论
本系统通过统一的 AI 客户端、Agent 工具协议、流式响应与三阶段持久化，结合 ACE 反思沉淀策略，实现了高效、可靠、可演进的 AI 助手能力。多模型配置与安全机制保障了灵活性与安全性；HNSW 向量检索与混合检索提升了语义理解与历史回溯能力。开发者可基于 MemoryStore 接口扩展存储实现，基于 Agent 协议扩展工具与编辑操作，基于 Reflector 扩展策略沉淀规则。
...
- 使用示例（配置与调用）
  ...
  - 语义搜索：WarmDataStore.semantic_search(query_text, k) 返回相关会话元数据。
