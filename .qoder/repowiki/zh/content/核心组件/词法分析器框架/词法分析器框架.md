...
## 更新摘要
**所做更改**
- 移除了所有词法分析器中的复杂tree-sitter集成代码
- 采用纯Rust实现的自定义解析逻辑，显著提升性能
- 优化了内存管理和增量分析效率
- 简化了架构设计，提高了可维护性
...
**重要更新**：当前实现已完全移除复杂的tree-sitter集成，采用纯Rust自定义解析逻辑，显著提升了性能和可维护性。
...
subgraph "语言实现"
CLexer["c_lexer.rs<br/>纯Rust DFA解析"]
RustLex["rust_lexer.rs<br/>纯Rust DFA解析"]
PyLex["python_lexer.rs<br/>纯Rust DFA解析"]
JsLex["js_lexer.rs<br/>纯Rust DFA解析"]
JsonLex["json_lexer.rs<br/>纯Rust DFA解析"]
MdLex["markdown_lexer.rs<br/>纯Rust DFA解析"]
TomlLex["toml_lexer.rs<br/>纯Rust DFA解析"]
HtmlLex["html_lexer.rs<br/>纯Rust DFA解析"]
end
subgraph "性能与基准"
...
- [crates/aether-core/src/lexer/mod.rs:160-196](file://crates/aether-core/src/lexer/mod.rs#L160-L196)
...
- **纯Rust实现优势**：移除了tree-sitter依赖，减少了外部依赖和编译时间，提升了启动速度和运行时性能。
...
- **纯Rust实现注意事项**：由于移除了tree-sitter，某些复杂语法可能需要更精细的手动处理。
...
- [crates/aether-core/src/lexer/c_lexer.rs:185-233](file://crates/aether-core/src/lexer/c_lexer.rs#L185-L233)
- [crates/aether-core/src/lexer/rust_lexer.rs:327-374](file://crates/aether-core/src/lexer/rust_lexer.rs#L327-L374)
- [crates/aether-core/src/lexer/js_lexer.rs:359-408](file://crates/aether-core/src/lexer/js_lexer.rs#L359-L408)
- [crates/aether-core/src/incremental_lexer.rs:43-101](file://crates/aether-core/src/incremental_lexer.rs#L43-L101)

## 结论
该词法分析器框架通过统一接口与丰富 Token 类型，实现了跨语言的高一致性与可扩展性；增量缓存显著提升了交互性能；SIMD 工具与基准测试为持续优化提供了支撑。**最重要的改进是移除了复杂的tree-sitter集成，采用纯Rust自定义解析逻辑，大幅提升了性能和可维护性**。未来可在更多语言上复用 common 工具与增量策略，并结合 SIMD 进一步优化热点路径。
