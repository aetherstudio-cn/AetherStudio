# EditorState 字段路径批量迁移脚本 v7
# 处理 state.field 模式（RefMut<EditorState> 直接引用）
...
Get-ChildItem -Path $srcDir -Recurse -Filter "*.rs" | ForEach-Object {
    ...
    foreach ($field in $sortedFields) {
        ...
        # 匹配 state.field 但不匹配 state.domain.field 或 state.field_xxx
        # 也不匹配 state.borrow() 或 state.borrow_mut()
        $pattern = "(?<![.\w])state\.$field(?![\w(])"
        $replacement = "state.$domain.$field"
