# EditorState 字段路径批量迁移脚本 v5
# 处理所有文件中的 st.field 模式（Ref/RefMut<EditorState>）
...
Get-ChildItem -Path $srcDir -Recurse -Filter "*.rs" | ForEach-Object {
    ...
    foreach ($field in $sortedFields) {
        ...
        # 匹配 st.field 但不匹配 st.domain.field 或 st.field_xxx
        $pattern = "(?<![.\w])st\.$field(?![\w])"
        $replacement = "st.$domain.$field"
