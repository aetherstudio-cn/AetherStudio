# EditorState 字段路径批量迁移脚本 v6
# 处理 state.borrow().field 和 state.borrow_mut().field 模式
...
Get-ChildItem -Path $srcDir -Recurse -Filter "*.rs" | ForEach-Object {
    ...
    foreach ($field in $sortedFields) {
        ...
        # 匹配 state.borrow().field 和 state.borrow_mut().field
        $pattern = "(?<![.\w])state\.borrow(?:_mut)?\(\)\.$field(?![\w])"
        $replacement = "state.borrow`$1().$domain.$field"
        # 简化：直接替换为 borrow().domain.field 或 borrow_mut().domain.field
        $newContent = [regex]::Replace($content, $pattern, { param($m)
            $m.Value -replace "\.$field$", ".$domain.$field"
        })
